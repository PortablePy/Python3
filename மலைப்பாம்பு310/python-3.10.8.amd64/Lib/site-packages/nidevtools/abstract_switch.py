...
class Session:
    ...
    def ss_connect(self, tsm: SMContext):
        ...
        if self.instrument_type == InstrumentTypes.daqmx:
            ...
            multiple_session.sessions[0].Task.write(bool(int(self.route_value)), True)
    ...
    def ss_read_state(self, tsm: SMContext):
        ...
        if self.instrument_type == InstrumentTypes.daqmx:
            ...
            multiple_session_info = fpga.pins_to_sessions(tsm, [self.enable_pin], [])
            data = multiple_session_info.read_static()[1]
            text = "Disconnected"
            if data[0]:
                text = "Connected"
            self.status = text
...
def check_debug_ui_tool(
    path_in: str,
    path_teststand="C:\\Users\\Public\\Documents\\National Instruments\\TestStand 2021 (64-bit)",
):
    """
...
def initialize(tsm: SMContext):  # CHECK
    ...
    ni_dt_common.g_tsm = tsm


@nitsm.codemoduleapi.code_module
def pin_fgv(
    tsm: SMContext, pin: str = "", action: Control = Control.get_connections, lock: Lock = None
):
    """
            ...
            if connection[1] == "AbstractInstrument":
                try:
                    if lock is not None:
                        ...
                        lock.acquire()
                    abs_sess_obj = pins_to_sessions_sessions_info(tsm, connection[0])
                    if lock is not None:
                        lock.release()
                    data = abs_sess_obj.read_state(tsm)  ## TODO reduce time
                finally:
                    if lock is not None:
                ...
                for info in data:
                    if info.instrument_type == InstrumentTypes.daqmx:
                        if info.route_value == "1" or info.route_value == "True":
                            condition = info.status.upper() == "TRUE"
                        else:
                            condition = info.status.upper() == "FALSE"
                    elif info.instrument_type == InstrumentTypes.digitalpattern:
                        if info.route_value == "1":
                            condition = info.status == "1" or info.status == "H"
                        else:
                            condition = info.status != "1" and info.status != "H"
...
@nitsm.codemoduleapi.code_module
def pin_fgv2(tsm: SMContext, pin: str = "", action: Control = Control.get_connections):
    """
        Pin switch status fgv labview equivalent code
    ...
    if action == Control.get_connections:
        ...
        for connection in pin_fgv.connections:
            if connection[1] == "AbstractInstrument":
                abs_sess_obj = pins_to_sessions_sessions_info(tsm, connection[0])  # 8ms for 8 pins
                sessions = sessions + abs_sess_obj.enable_pins
        sessions = AbstractSession(sessions)
        reading = sessions.read_state(tsm)
        i = 0
        for connection in pin_fgv.connections:
            if connection[1] == "AbstractInstrument":
                data = reading[i]  # 23 ms for 8 pins
                i = i + 1
                conditions = []
                condition = False
                for info in [data]:
...
def pin_name_to_instrument(pinmap_path: str = ""):
    ...
    multiplexed_connection = get_all_matched_nodes(
        connections, "MultiplexedConnection"
    )  # getting all MultiplexedConnection tags
    ...
    for element in multiplexed_connections_list:
        ...
        for j in connections_list:
            if j[0] == out1:
                ...
                r += 1
        try:
            out2 = "%s, %s" % (connections_list[r][1], connections_list[r][2])
        except IndexError:
            out2 = " - "
