...
T_Conf = const.TerminalConfiguration

try:
    T_C_DIFF = T_Conf.DIFFERENTIAL
except AttributeError:
    T_C_DIFF = T_Conf.DIFF
...
class _Session(typing.NamedTuple):
    ...
    # Read
    def st_read_wave_multi_chan(self, samples_per_chan=nidaqmx.task.NUM_SAMPLES_UNSET, timeout=10):
        """
        ...
        Return:
            Array of data
        """
        data = self.Task.read(samples_per_chan, timeout)
        return data

    def st_read_wave_single_chan(self, samples_per_chan=nidaqmx.task.NUM_SAMPLES_UNSET, timeout=10):
        """
        Reads one or more waveforms from the task specified in the session that contains
        one or more analog input channels.
        Return:
...
class _Sessions:
    ...
    def read_waveform_multichannel(
        self, samples_per_chan=nidaqmx.task.NUM_SAMPLES_UNSET, timeout=10
    ):
        """
        Reads one or more waveforms from each task specified in the list of session that contains
        one or more analog input channels.
        Args:
            samples_per_chan: Specifies the number of samples to read. If this input is not set,
        ...
        for session in self.sessions:
            data = session.st_read_wave_multi_chan(samples_per_chan, timeout)
            waveform += data
        return waveform
    ...
    def reference_analog_edge(
        ...
        Args:
            trigger_src: (String) is the name of a virtual channel or terminal where there is an
                analog signal to use as the source of the trigger. For E Series devices, if you use
                a virtual channel, it must be the only channel in the task. The only terminal you
                can use for E Series devices is PFI0.
            edge: (Enum) Specifies on which edge of the signal the reference trigger occurs.
        ...
        """
        for session in self.sessions:
            session.st_ref_analog_edge(trigger_src, edge, level_v, pre_trigger_samples_per_channel)
...
def _open_ai_sessions(
    tsm: SMContext, lock: Lock, task_type: str, term_config: T_Conf = T_Conf.DEFAULT, v_range=10.0
):
    if lock is not None:
        lock.acquire()
    task_names, channel_lists = tsm.get_all_nidaqmx_task_names(task_type)
    if lock is not None:
        lock.release()
    errors = []
    for task_name, phy_ch in zip(task_names, channel_lists):
        task = nidaqmx.Task(task_name)
        try:
            task.ai_channels.add_ai_voltage_chan(phy_ch, "", term_config, -v_range, v_range)
            task.timing.samp_timing_type = const.SampleTimingType.SAMPLE_CLOCK
            if lock is not None:
                with lock:
...
def _open_ds_ai_sessions(tsm: SMContext, lock: Lock, task_type: str):
    if lock is not None:
        lock.acquire()
    task_names, channel_lists = tsm.get_all_nidaqmx_task_names(task_type)
    if lock is not None:
    ...
    for task_name, phy_ch in zip(task_names, channel_lists):
        task = nidaqmx.Task(task_name)
        try:
            _ = task.ai_channels.add_ai_voltage_chan(phy_ch)  # returns channel for properties
            task.timing.samp_timing_type = const.SampleTimingType.SAMPLE_CLOCK
            if lock is not None:
                with lock:
                    tsm.set_nidaqmx_task(task_name, task)
...
def _open_ao_sessions(tsm: SMContext, lock: Lock, task_type: str):
    if lock is not None:
        lock.acquire()
    task_names, channel_lists = tsm.get_all_nidaqmx_task_names(task_type)
    if lock is not None:
    ...
    for task_name, phy_ch in zip(task_names, channel_lists):
        task = nidaqmx.Task(task_name)
        try:
            task.ao_channels.add_ao_voltage_chan(phy_ch)
            task.timing.samp_timing_type = const.SampleTimingType.SAMPLE_CLOCK
            if lock is not None:
                with lock:
                    tsm.set_nidaqmx_task(task_name, task)
...
def _open_ds_ao_sessions(
    tsm: SMContext, lock: Lock, task_type: str, term_config: T_Conf = T_Conf.DEFAULT
):
    if lock is not None:
        lock.acquire()
    task_names, channel_lists = tsm.get_all_nidaqmx_task_names(task_type)
    ...
    for task_name, phy_ch in zip(task_names, channel_lists):
        task = nidaqmx.Task(task_name)
        try:
            ch = task.ao_channels.add_ao_voltage_chan(phy_ch)
            task.timing.samp_timing_type = const.SampleTimingType.SAMPLE_CLOCK
            ch.ao_term_cfg = term_config
            if lock is not None:
                with lock:
                    tsm.set_nidaqmx_task(task_name, task)
            else:
...
def _open_do_sessions(tsm: SMContext, lock: Lock, task_type: str):
    if lock is not None:
        lock.acquire()
    task_names, channel_lists = tsm.get_all_nidaqmx_task_names(task_type)
    if lock is not None:
    ...
    for task_name, phy_ch in zip(task_names, channel_lists):
        task = nidaqmx.Task(task_name)
        try:
            task.do_channels.add_do_chan(phy_ch, "", const.LineGrouping.CHAN_PER_LINE)
            task.timing.samp_timing_type = const.SampleTimingType.ON_DEMAND
            if lock is not None:
                with lock:
                    tsm.set_nidaqmx_task(task_name, task)
            else:
...
def _close_sessions(tsm: SMContext, lock: Lock, task_type: str):
    """
    closes the sessions  of the give task type

    Args:
        tsm (SMContext): TestStand semiconductor module context
        task_type:  identification of task type string constant
...
def set_task(tsm: SMContext, lock: Lock = None):
    """
    ...
    if lock is not None:
        ...
        # lock.acquire()
    errors.extend(_open_ai_sessions(tsm, lock, "AnalogInput", T_C_DIFF, 10.0))
    errors.extend(_open_ds_ai_sessions(tsm, lock, "AnalogInputDSA"))
    errors.extend(_open_ao_sessions(tsm, lock, "AnalogOutput"))
    errors.extend(_open_do_sessions(tsm, lock, "DigitalOutput"))
    errors.extend(_open_ds_ao_sessions(tsm, lock, "AnalogOutputDSA", T_C_DIFF))
    if lock is not None:
...
@nitsm.codemoduleapi.code_module
def clear_task(tsm: SMContext, lock: Lock = None):
    """
    ...
    if lock is not None:
        pythoncom.CoInitialize()
    _close_sessions(tsm, lock, "AnalogInput")
    _close_sessions(tsm, lock, "AnalogInputDSA")
    _close_sessions(tsm, lock, "AnalogOutput")
    _close_sessions(tsm, lock, "DigitalOutput")
    _close_sessions(tsm, lock, "AnalogOutputDSA")
    if lock is not None:
        pythoncom.CoUninitialize()
