...
from nidigital.enums import DriveFormat as DrFmt, TerminationMode as Term
import nidigital
...
class DigiTiming:
    def __init__(self, name: str, period: float, fmt: DrFmt, on: float, data: float, back: float,
                 off: float, compare: float):
        self.name = name
        self.period = period
        self.fmt = fmt
        self.on = on
...
class DigiLevels:
    def __init__(self, vih: float, vil: float, voh: float, vol: float, term: Term, vterm: float,
                 vcom: float, iol: float, ioh: float):
        self.vih = vih
        self.vil = vil
        self.voh = voh
        self.vol = vol
...
class _NIDigitalSSC:
    ...
    def __init__(self, session: nidigital.Session, pins: str, site: str):
        """
        ...
        Args:
            ...
            pins (str): specific channel(s) of that session for this context.
            site (str): pins (names) mapped to the channels.
        """
        self._ses = session
        self._pins = pins
        self._site = site
        self._ch_ses = session.channels[pins]  # session with specific channel(s)
    ...
    def cs_clock_generator_abort(self):
        ...
        """
        return self._ch_ses.clock_generator_abort()
    ...
    def cs_clock_generator_generate_clock(self, frequency: float, select_digital: bool = True):
        ...
        Returns:
            none: when successful otherwise exception will be thrown.
        """
        return self._ch_ses.clock_generator_generate_clock(frequency, select_digital)
    ...
    def cs_modify_time_set_for_clock_generation(
        ...
        """
        period = 1 / frequency
        self._ses.configure_time_set_period(time_set, period)
        self._ch_ses.configure_time_set_drive_edges(
            time_set,
    ...
    def as_load_pattern(self, pattern_file):
        """
        as_load_pattern
        ...
        Args:
            pattern_file (str): Absolute file path of the binary .digipat pattern file to load.
            Specify the pattern to burst using start_label or the start_label parameter of the burst_pattern method.

        """
        self._ses.unload_all_patterns()
        self._ses.load_pattern(pattern_file)
    ...
    def cs_select_function(self, function: enums.SelectedFunction):
        ...
        """
        self._ses.abort()
        self._ch_ses.selected_function = function
    ...
    def cs_frequency_counter_configure_measurement_time(self, measurement_time: float):
        ...
        """
        self._ch_ses.frequency_counter_measurement_time = measurement_time
    ...
    def cs_frequency_counter_measure_frequency(self):
        ...
        """
        return self._ch_ses.frequency_counter_measure_frequency()
    ...
    def cs_configure_hram_settings(
        ...
        """
        self._ses.history_ram_cycles_to_acquire = cycles_to_acquire
        self._ses.history_ram_pretrigger_samples = pretrigger_samples
        self._ses.history_ram_max_samples_to_acquire_per_site = max_samples_to_acquire_per_site
        self._ses.history_ram_number_of_samples_is_finite = number_of_samples_is_finite
        self._ses.history_ram_buffer_size_per_site = buffer_size_per_site
    ...
    def cs_configure_hram_trigger(
        ...
        if triggers_type == enums.HistoryRAMTriggerType.FIRST_FAILURE:
            self._ses.history_ram_trigger_type = triggers_type
        elif triggers_type == enums.HistoryRAMTriggerType.CYCLE_NUMBER:
            self._ses.history_ram_trigger_type = triggers_type
            self._ses.cycle_number_history_ram_trigger_cycle_number = cycle_number
        elif triggers_type == enums.HistoryRAMTriggerType.PATTERN_LABEL:
            self._ses.history_ram_trigger_type = triggers_type
            self._ses.pattern_label_history_ram_trigger_label = pattern_label
            self._ses.pattern_label_history_ram_trigger_cycle_offset = cycle_offset
            self._ses.pattern_label_history_ram_trigger_vector_offset = vector_offset
    ...
    def cs_get_hram_settings(self):
        ...
        """
        return (
            self._ses.history_ram_cycles_to_acquire,
            self._ses.history_ram_pretrigger_samples,
            self._ses.history_ram_max_samples_to_acquire_per_site,
            self._ses.history_ram_number_of_samples_is_finite,
            self._ses.history_ram_buffer_size_per_site,
        )
    ...
    def cs_get_hram_trigger_settings(self):
        ...
        """
        return (
            self._ses.history_ram_trigger_type,
            self._ses.cycle_number_history_ram_trigger_cycle_number,
            self._ses.pattern_label_history_ram_trigger_label,
            self._ses.pattern_label_history_ram_trigger_cycle_offset,
            self._ses.pattern_label_history_ram_trigger_vector_offset,
        )
    ...
    def cs_get_fail_count(self):
        ...
        Returns:
            ...
            channels.
        """
        return self._ch_ses.get_fail_count()
    ...
    def cs_wait_until_done(self, timeout: float = 10):
        ...
        """
        self._ses.wait_until_done(timeout)
    ...
    def cs_apply_tdr_offsets(self, per_instrument_offset: typing.List[float]):
        ...
        **applyOffsets** input of the tdr method was set to False at the time of measurement.
        ...
        """
        self._ch_ses.apply_tdr_offsets(per_instrument_offset)

    def cs_configure_voltage_levels(self, vil: float, vih: float, vol: float, voh: float, vterm: float):
        """
        ...
        """
        self._ch_ses.configure_voltage_levels(vil, vih, vol, voh, vterm)
    ...
    def cs_configure_termination_mode(self, termination_mode: enums.TerminationMode):
        ...
        +-----------------------------+------------------------------------------------------------+
        """
        self._ch_ses.termination_mode = termination_mode

    def cs_configure_active_load(self, iol: float, ioh: float, vcom: float):
        """
        ...
        Args:
            vcom (float): Commutation voltage level at which the active load circuit
            ...
            above V\ :sub:`COM`.
        """
        self._ch_ses.configure_active_load_levels(iol, ioh, vcom)

    def cs_configure_time_set_drive_edges(self, name, d_format, drive_on_edge,
                                          drive_data_edge, drive_return_edge, drive_off_edge):
        """
        Configures the drive format and drive edge placement for the specified pins.
        Use this method to modify time set values after applying a timing sheet with the
        apply_levels_and_timing method, or to create time sets programmatically without
        the use of timing sheets. This method does not modify the timing sheet file or
        the timing sheet contents that will be used in future calls to apply_levels_and_timing;
        it only affects the values of the current timing context.

        Args:
            name (str): The specified time set name.
            d_format (enums.DriveFormat): Drive format of the time set.
            ...
            drive_on_edge (hightime.timedelta, datetime.timedelta, or float in seconds):
                Delay, in seconds, from the beginning of the vector period for turning on the pin
                driver.This option applies only when the prior vector left the pin in a non-drive
                pin state (L, H, X, V, M, E). For the SBC format, this option specifies the delay
                from the beginning of the vector period at which the complement of the pattern
                value is driven.
            drive_data_edge (hightime.timedelta, datetime.timedelta, or float in seconds):
                Delay, in seconds, from the beginning of the vector period until the pattern data
                is driven to the pattern value.The ending state from the previous vector persists
                until this point.
            drive_return_edge (hightime.timedelta, datetime.timedelta, or float in seconds): Delay,
                in seconds, from the beginning of the vector period until the pin changes from the
                pattern data to the return value, as specified in the format.
            drive_off_edge (hightime.timedelta, datetime.timedelta, or float in seconds): Delay, in
                seconds, from the beginning of the vector period to turn off the pin driver when
                the next vector period uses a non-drive symbol (L, H, X, V, M, E).
        """
        self._ch_ses.configure_time_set_drive_edges(name, d_format, drive_on_edge,
    ...
    def cs_configure_time_set_compare_edge_per_pin(
        ...
        Args:
            ...
            period.
        """
        channels, _, _ = _channel_list_to_pins(self._pins)
        for channel, compare_strobe in zip(channels, compare_strobes):
            self._ch_ses.configure_time_set_compare_edges_strobe(time_set, compare_strobe)
    ...
    def cs_read_static(self):
        ...
        """
        return self._ch_ses.read_static()
    ...
    def cs_write_static(self, state: enums.WriteStaticPinState, auto_select=True):
        ...
        self._ch_ses.write_static(state)
    ...
    def cs_get_properties(self):  # checked _
        ...
        """
        instrument_name = self._ses.io_resource_descriptor
        # match = re.search(r"[A-Za-z]+[1-9]+", str(self._ses.io_resource_descriptor))
        # if match:
        ...
        session_property = SessionProperties(
            ...
            self._ch_ses.vil,
            self._ch_ses.vterm,
            self._ch_ses.frequency_counter_measurement_time,
        )
    ...
    def cs_clear_start_trigger_signal(self):
        ...
        """
        self._ses.start_trigger_type = enums.TriggerType.NONE
    ...
    def cs_configure_trigger_signal(
        ...
        """
        self._ses.digital_edge_start_trigger_source = source
        self._ses.digital_edge_start_trigger_edge = edge
        self._ses.start_trigger_type = enums.TriggerType.DIGITAL_EDGE
    ...
    def cs_export_opcode_trigger_signal(self, signal_id: str, terminal: str = ""):
        ...
        """
        self._ses.pattern_opcode_events[
            signal_id
    ...
    def cs_ppmu_source_current(self, current_level: float, current_level_range: float = 0):
        ...
        self._ch_ses.ppmu_output_function = enums.PPMUOutputFunction.CURRENT
        self._ch_ses.ppmu_current_level_range = current_level_range
        self._ch_ses.ppmu_current_level = current_level
        self.cs_ppmu_source()
    ...
    def cs_ppmu_source_voltage(self, voltage_level: float, current_limit_range: float):
        ...
        self._ch_ses.ppmu_output_function = enums.PPMUOutputFunction.VOLTAGE
        self._ch_ses.ppmu_current_limit_range = current_limit_range
        self._ch_ses.ppmu_voltage_level = voltage_level
        self.cs_ppmu_source()

    def cs_create_capture_waveform_serial(self, wfm, sample_width, bit_order, pins):
        """
        Sets the capture waveform settings for serial acquisition. Settings apply across all sites if multiple sites
        are configured in the pin map. You cannot reconfigure settings after waveforms are created.

        Args:
            wfm (str): Waveform name you want to use. Use the waveform_name with the capture_start opcode
            in your pattern.
            ...
            pins (str): Pins for which capture waveform is created
        """
        self._ses.pins[pins].create_capture_waveform_serial(wfm, sample_width, bit_order)

    def cs_create_source_waveform_serial(self, wfm, data_mapping, sample_width, bit_order, pins):
        """
        Sets the source waveform settings required for serial sourcing. Settings apply across all sites if multiple
        sites are configured in the pin map. You cannot reconfigure settings after waveforms are created.

        Args:
            wfm (str): The name to assign to the waveform. Use the waveform_name  with source_start opcode
            in your pattern.
            ...
            bit_order (enums.BitOrder): Order in which to shift the bits.
                -   BitOrder.MSB: Specifies the bit order by most significant bit first.
                -   BitOrder.LSB: Specifies the bit order by least significant bit first.
            pins (str): Pins for which source waveform is created
        """
        self._ses.pins[pins].create_source_waveform_serial(
            wfm, data_mapping, sample_width, bit_order
        )
    ...
    def ps_burst_pattern(
        ...
        """
        self._ses.sites[self._site].burst_pattern(
    ...
    def ps_get_site_pass_fail(self):
        ...
        """
        return list(self._ses.sites[self._site].get_site_pass_fail().values())
...
class _NIDigitalTSM:
    ...
    def stream_hram_results(self):
        ...
        Returns:
            ...
            class instances with the following information about each pattern cycle:
                ...
                -  **name** (str) Time set for the acquired cycle.
        ...
        for ssc in self._sscs:
            ...
            for channel, site in zip(channel_list_array, site_list_array):
                per_instrument_per_site_array.append(_NIDigitalSSC(ssc._ses, channel, site))
        ...
        for ssc in per_instrument_per_site_array:
            ...
            stop = False
            while not stop:
                done = ssc._ses.is_done()
                _, pins, sites = _channel_list_to_pins(ssc._pins)
                sample_count = ssc._ses.sites[ssc._site].get_history_ram_sample_count()
                samples_to_read = sample_count - read_position
                cycle_information += (
                    ssc._ses.sites[sites]
                    .pins[pins]
    ...
    def load_pattern(self, pattern_file):
        ...
        """
        for ssc in self._sscs:
            ssc.as_load_pattern(pattern_file)
    ...
    def apply_digitiming(self, dt: DigiTiming, create_time_set=False):
        """
        Configure time set through driver API
        args:
            dt - DigiTiming same order of parameters as in digitiming file
        """
        try:
            if create_time_set:
                self.create_time_set(dt.name)  # Must match time set name in pattern file
        except Exception:
            pass  # ignore the time_set creation error
        self.configure_time_set_period(dt.name, dt.period)
        self.configure_time_set_drive_edges(dt.name, dt.fmt, dt.on, dt.data, dt.back, dt.off)
        self.configure_time_set_compare_edge(dt.name, dt.compare)

    def apply_digilevels(self, dl: DigiLevels):
        """
        Configure levels through driver API
        args:
            dl - DigiLevels same order of parameters as in digiLevels file
        """
        self.configure_voltage_levels(dl.vil, dl.vih, dl.vol, dl.voh, dl.vterm)
        self.configure_termination_mode(dl.term)
        if dl.term == Term.ACTIVE_LOAD:
            self.configure_active_load(dl.iol, dl.ioh, dl.vcom)
    ...
    def apply_levels_and_timing(self, levels_sheet: str, timing_sheet: str):
        ...
        """
        for ssc in self._sscs:
            ssc._ses.apply_levels_and_timing(levels_sheet, timing_sheet)
    ...
    def configure_voltage_levels(self, vil: float, vih: float, vol: float, voh: float, vterm: float):
        """
        Configures voltage levels for the pins in context.
        ...
        """
        for ssc in self._sscs:
            ssc.cs_configure_voltage_levels(vil, vih, vol, voh, vterm)
    ...
    def configure_single_level_per_site(
        ...
        for ssc, settings in zip(self._sscs, per_site_value):
            channel_list_array, _, _ = _arrange_channels_per_site(ssc._pins, ssc._site)
            for channel, setting in zip(channel_list_array, settings):
                if level_type_to_set == LevelTypeToSet.VIL:
                    ssc._ses.channels[channel].vil = setting
                elif level_type_to_set == LevelTypeToSet.VIH:
                    ssc._ses.channels[channel].vih = setting
                elif level_type_to_set == LevelTypeToSet.VOL:
                    ssc._ses.channels[channel].vol = setting
                elif level_type_to_set == LevelTypeToSet.VOH:
                    ssc._ses.channels[channel].voh = setting
                elif level_type_to_set == LevelTypeToSet.VTERM:
                    ssc._ses.channels[channel].vterm = setting
                elif level_type_to_set == LevelTypeToSet.IOL:
                    ssc._ses.channels[channel].active_load_iol = setting
                elif level_type_to_set == LevelTypeToSet.IOH:
                    ssc._ses.channels[channel].active_load_ioh = setting
                elif level_type_to_set == LevelTypeToSet.VCOM:
                    ssc._ses.channels[channel].active_load_vcom = setting
                ssc._ses.commit()
    ...
    def configure_active_load(self, iol: float, ioh: float, vcom: float):
        """
    ...
    def create_time_set(self, name: str):
        """
        Creates a time set with the name that you specify. Use this method when you want to
        create time sets programmatically rather than with a timing sheet.

        Args:
            name (str): The specified name of the new time set.
        """
        for ssc in self._sscs:
            ssc._ses.create_time_set(name)
    ...
    def configure_time_set_period(self, time_set: str, period: float):
        ...
        for ssc in self._sscs:
            ssc._ses.configure_time_set_period(time_set, configured_period)
        return configured_period

    def configure_time_set_drive_edges(self, name, d_format, drive_on_edge, drive_data_edge,
        ...
        """
        for ssc in self._sscs:
            ssc.cs_configure_time_set_drive_edges(name, d_format, drive_on_edge, drive_data_edge,
                                                  drive_return_edge, drive_off_edge)
    ...
    def configure_time_set_compare_edge_per_site(self, time_set: str, per_site_compare_strobe:
        ...
        for ssc, compare_strobes in zip(self._sscs, per_site_compare_strobe):
            ...
            for channel, compare_strobe in zip(channel_list_array, compare_strobes):
                ssc._ses.channels[channel].configure_time_set_compare_edges_strobe(
                    time_set, compare_strobe
                )

    def configure_time_set_compare_edge_per_site_per_pin(self, time_set: str,
    ...
    def ppmu_configure_aperture_time(self, aperture_time: float):
        ...
        """
        for ssc in self._sscs:
            ssc._ses.channels[ssc._pins].ppmu_aperture_time = aperture_time
    ...
    def ppmu_configure_current_limit_range(self, current_limit_range: float):
        ...
        current_limit_range = abs(current_limit_range)
        for ssc in self._sscs:
            ssc._ses.channels[ssc._pins].ppmu_current_limit_range = current_limit_range
    ...
    def ppmu_configure_voltage_limits(self, voltage_limit_high: float, voltage_limit_low: float):
        ...
        """
        for ssc in self._sscs:
            ssc._ses.channels[ssc._pins].ppmu_voltage_limit_high = voltage_limit_high
            ssc._ses.channels[ssc._pins].ppmu_voltage_limit_low = voltage_limit_low
    ...
    def ppmu_measure(self, measurement_type: enums.PPMUMeasurementType):
        ...
        for ssc in self._sscs:
            per_instrument_measurements.append(
                ssc._ses.channels[ssc._pins].ppmu_measure(measurement_type)
            )
    ...
    def ppmu_source_voltage_per_site_per_pin(
        ...
        for ssc, source_voltages in zip(self._sscs, per_site_per_pin_source_voltages):
            ssc._ses.channels[ssc._pins].ppmu_output_function = enums.PPMUOutputFunction.VOLTAGE
            ssc._ses.channels[ssc._pins].ppmu_current_limit_range = current_limit_range
            channels, _, _ = _channel_list_to_pins(ssc._pins)
            for channel, source_voltage in zip(channels, source_voltages):
                ssc._ses.channels[channel].ppmu_voltage_level = source_voltage
            ssc._ses.channels[ssc._pins].ppmu_source()
    ...
    def create_capture_waveform_serial(
        self, waveform_name: str, sample_width: int, bit_order: enums.BitOrder, pins
    ):
        """
        Sets the capture waveform settings for serial acquisition. Settings apply across all sites if multiple sites
        are configured in the pin map. You cannot reconfigure settings after waveforms are created.
        ...
        Args:
            waveform_name (str): Waveform name you want to use. Use the waveform_name with the capture_start opcode
            in your pattern.
            sample_width (int): Width in bits of each serial sample. Valid values are between 1 and 32.
            bit_order (enums.BitOrder): Order in which to shift the bits.
                -   BitOrder.MSB: Specifies the bit order by most significant bit first.
        ...
        """
        for ssc in self._sscs:
            ssc.cs_create_capture_waveform_serial(waveform_name, sample_width, bit_order, pins)

    def create_source_waveform_serial(
        self, waveform_name, data_mapping, sample_width, bit_order, pins
    ):
        """
        Sets the source waveform settings required for serial sourcing. Settings apply across all sites if multiple
         sites are configured in the pin map. You cannot reconfigure settings after waveforms are created.
        ...
        Args:
            waveform_name (str): The name to assign to the waveform. Use the waveform_name  with source_start opcode in
            your pattern.
            data_mapping (enums.SourceDataMapping): Parameter that specifies how to map data on multiple sites.
                -   SourceDataMapping.BROADCAST: Broadcasts the waveform you specify to all sites.
                -   SourceDataMapping.SITE_UNIQUE: Sources unique waveform data to each site.
        ...
        """
        for ssc in self._sscs:
            ssc.cs_create_source_waveform_serial(
                waveform_name, data_mapping, sample_width, bit_order, pins
            )
    ...
    def read_sequencer_flag(self, sequencer_flag: enums.SequencerFlag):
        ...
        for ssc in self._sscs:
            per_instrument_state.append(ssc._ses.read_sequencer_flag(sequencer_flag))
    ...
    def read_sequencer_register(self, sequencer_register: enums.SequencerRegister):
        ...
        for ssc in self._sscs:
            per_instrument_register_values.append(
                ssc._ses.read_sequencer_register(sequencer_register)
            )
    ...
    def write_sequencer_flag(
        ...
        """
        for ssc in self._sscs:
            ssc._ses.write_sequencer_flag(sequencer_flag, state)
    ...
    def write_sequencer_register(
        ...
        """
        for ssc in self._sscs:
            ssc._ses.write_sequencer_register(sequencer_register, value)
    ...
    def fetch_capture_waveform(
        ...
        Returns:
            per_instrument_capture: typing.List[typing.List[typing.List[int]]] ({ int: memory view of array.
            array of unsigned int,
                int:memory view of array. array of unsigned int, ... }): Dictionary where each
                key is a site number and value is a collection of digital states representing
                capture waveform data
        """
        per_instrument_capture: typing.List[typing.List[typing.List[int]]] = []
        for ssc in self._sscs:
            waveforms = ssc._ses.sites[ssc._site].fetch_capture_waveform(
    ...
    def write_source_waveform_broadcast(
        ...
        for ssc in self._sscs:
            ssc._ses.write_source_waveform_broadcast(waveform_name, waveform_data)
    ...
    def write_source_waveform_site_unique(
        ...
        for ssc, per_instrument_waveform in zip(self._sscs, per_instrument_waveforms):
            ...
            site_numbers, _ = _site_list_to_site_numbers(ssc._site)
            ...
            ssc._ses.write_source_waveform_site_unique(waveform_name, waveform_data)
        ...
        for ssc, states in zip(self._sscs, per_site_per_pin_state):
            ...
            for channel, state in zip(channels, states):
                if auto_select:
                    ssc._ses.channels[
                        channel
                    ].selected_function = enums.SelectedFunction.DIGITAL
                ssc._ses.channels[channel].write_static(state)
    ...
    def filter_sites(self, desired_sites: typing.List[int]):
        ...
        for ssc in self._sscs:
            ...
            if site_list:
                sscs_new.append(
                    _NIDigitalSSC(ssc._ses, ",".join(channel_list), ",".join(site_list))
                )
    ...
    def calculate_per_instrument_to_per_site_per_pin_lut(
        ...
        for ssc in self._sscs:
            _, _pins, _sites = _channel_list_to_pins(ssc._pins)
    ...
    def calculate_per_site_per_pin_to_per_instrument_lut(
        ...
        for _ssc in self._sscs:
            _, _pins, _sites = _channel_list_to_pins(_ssc._pins)
    ...
    def get_properties(self):
        ...
        frequency_counter_measurement_time in a named tuple d_format
...
class TSMDigital:
    ...
    def get_hram_configuration(self):
        """
        gets the current HRAM settings and HRAM trigger settings in a tuple of lists d_format
    ...
    def per_instrument_to_per_site_data(self, per_instrument_data):
        """
        data transform as dictionary.
        Args:
            per_instrument_data (any): data to be transformed.

        Returns:
            List of data ({ int: any, int: any, ... }): Dictionary where each key is a site
                number and value is data for the site.
        """
        initialized_array = [None for _ in self.sites]
        per_instrument_to_per_site_lut = self.ssc.calculate_per_instrument_to_per_site_lut(
            self.sites
        )
        per_site_data = _apply_lut_per_instrument_to_per_site(
            initialized_array, per_instrument_to_per_site_lut, per_instrument_data
        )
        return per_site_data
...
def _site_list_to_site_numbers(site_list: str):
    ...
    Returns:
        site_numbers, sites: numbers list of sites and string list of sites
    """
    ...
    for site in sites:
        try:
            mat_var = re.match(r"site(\d+)", site.lower()).group(1)
        except Exception as e:
            mat_var = site
            print(e)
        numb = int(mat_var)
        site_numbers.append(numb)
...
def _channel_list_to_pins(channel_list: str):
    ...
    for i in range(len(channels)):
        try:
            ...
            sites[i] = int(re.match(r"site(\d+)", site.lower()).group(1))
...
def pins_to_sessions(
    ...
    Returns:
        TSMDigital: object containing pin-query context variable and other session details
    """
...
def initialize_sessions(tsm: SMContext, lock: Lock = None, options: dict = {}):
    ...
    Args:
        ...
        lock (None): instance of Threading.Lock()
        options (dic): Options for the function
    """
    try:
        if lock is not None:
