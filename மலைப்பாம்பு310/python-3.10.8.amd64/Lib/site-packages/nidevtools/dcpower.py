...
class CustomTransientResponse:
    ...
    def __init__(self, gain_bw: float, compensation_frequency: float, pole_zero_ratio: float):
        """
        constructor for storing custom transient response

        Args:
            gain_bw (float): gain bandwidth is stored as floating point number
...
class ChannelProperties(typing.NamedTuple):
    ...
    voltage_limit_h: str
    voltage_limit_l: str
    voltage_limit_range: str
    current_level: str
    current_limit: str
    current_limit_h: str
    current_limit_l: str
    current_limit_range: str
...
def model_to_ranges(model: int, channel: int):
    ...
    if model == 4110:
        ...
        current_ranges = [10e-6, 100e-6, 1e-3, 10e-3, 0.1]
        # Need to check the spec 0.06 may be missing
        voltage_ranges = [24]
    elif model == 4163:
...
class _NIDCPowerSSC:
    ...
    def __init__(self, session: nidcpower.Session, channels: str, pins: str):
        ...
        self.ch_session = session.channels[channels]
    ...
    def cs_session(self):
        ...
        """
        return self.ch_session  # This session will operate only on subset of channels
    ...
    def cs_initiate(self):
        ...
        """
        return self.ch_session.initiate()
    ...
    def cs_reset(self):
        ...
        Returns:
            None: when this operation is completed successfully, i.e. no error.
        """
        return self.ch_session.reset()

    def cs_config_aperture_restart(
        ...
        """
        self.ch_session.abort()
        self.ch_session.aperture_time = aperture_time
        self.ch_session.aperture_time_units = aperture_time_units
        self.ch_session.initiate()
    ...
    def cs_configure_aperture_time(
        ...
        Returns:
            None: when this operation is completed successfully, i.e. no error.
        """
        return self.ch_session.configure_aperture_time(aperture_time, aperture_time_units)
    ...
    def cs_configure_power_line_frequency(self, power_line_frequency=60.0):
        ...
        self.ch_session.power_line_frequency = power_line_frequency
    ...
    def cs_configure_settings(
        ...
        if match == "4110":
            self.ch_session.source_delay = source_delay
            self.ch_session.samples_to_average = 3000 * temp
        elif match == "4130":
            self.ch_session.source_delay = source_delay
            self.ch_session.samples_to_average = 3000 * temp
            if self.ch_session.channels == "1":
                self.ch_session.sense = sense
        elif match == "4154":
            self.ch_session.source_delay = source_delay
            self.ch_session.samples_to_average = 300000 * temp
            self.ch_session.sense = sense
            if self.ch_session.channels == "0":
                self.ch_session.transient_response = transient_response
        elif match == "4132":
            self.ch_session.aperture_time_units = aperture_time_unit
            self.ch_session.aperture_time = aperture_time
            self.ch_session.source_delay = source_delay
            self.ch_session.sense = sense
            self.ch_session.sense = sense
    ...
    def cs_get_aperture_time_in_seconds(self):
        ...
        actual_apt_t = self.ch_session.aperture_time_units
        if match in all_supported_models + ["4112", "4113", "4132"]:
            if self.ch_session.aperture_time_units == enums.ApertureTimeUnits.POWER_LINE_CYCLES:
                actual_apt_t = (
                    self.ch_session.aperture_time_units / self.ch_session.power_line_frequency
                )

        if match in ["4110", "4130"]:
            actual_apt_t = self.ch_session.samples_to_average / 3000
        elif match == "4154":
            actual_apt_t = self.ch_session.samples_to_average / 300000
        return actual_apt_t
    ...
    def cs_get_power_line_frequency(self):
        ...
        """
        match = re.search("\d\d\d\d", self._session.instrument_model, re.RegexFlag.ASCII)[0]
        configured_power_line_frequency = self.ch_session.power_line_frequency
        if match in ["4110", "4130"]:
    ...
    def cs_configure_current_level_range(self, current_level_range=0.0):
        ...
        """
        self.ch_session.current_level_range = current_level_range
    ...
    def cs_configure_current_level(self, current_level=0.0):
        ...
        """
        self.ch_session.current_level = current_level
    ...
    def cs_configure_single_pt_force_current_asymmetric_limits(
        ...
        """
        self.ch_session.source_mode = nidcpower.SourceMode.SINGLE_POINT
        self.ch_session.output_function = nidcpower.OutputFunction.DC_CURRENT
        self.ch_session.current_level = current_level
        self.ch_session.voltage_limit_high = voltage_limit_high
        self.ch_session.voltage_limit_low = voltage_limit_low
        c_value = current_level_range
        if c_value == 0.0:
            c_value = abs(current_level)
        self.ch_session.current_level_range = c_value
        v_value = voltage_limit_range
        if v_value == 0.0:
            v_value = max(abs(voltage_limit_high), abs(voltage_limit_low))
        self.ch_session.voltage_limit_range = v_value
        self.ch_session.compliance_limit_symmetry = nidcpower.ComplianceLimitSymmetry.ASYMMETRIC
    ...
    def cs_configure_single_pt_force_current_symmetric_limits(
        ...
        self.ch_session.output_function = nidcpower.OutputFunction.DC_CURRENT
        self.ch_session.current_level = current_level
        self.ch_session.voltage_limit = voltage_limit
        c_value = current_level_range
        if c_value == 0.0:
            c_value = abs(current_level)
        self.ch_session.current_level_range = c_value
        ...
        self.ch_session.voltage_limit_range = v_value
        self.ch_session.compliance_limit_symmetry = nidcpower.ComplianceLimitSymmetry.SYMMETRIC
    ...
    def cs_configure_voltage_limit_range(self, voltage_limit_range=0.0):
        ...
        the valid values to which the voltage limit can be set. Use the voltage_limit_auto-range
        ...
        Args:
            voltage_limit_range (float, optional): voltage limit range to set in volts. Defaults to
                0.0.
        """
        self.ch_session.voltage_limit_range = voltage_limit_range
    ...
    def cs_configure_current_limit_range(self, current_limit_range=0.0):
        ...
        valid value to which the current limit can be set. Use the current_limit_auto-range property
        ...
        """
        self.ch_session.current_limit_range = current_limit_range
    ...
    def cs_configure_current_limit(self, current_limit=0.0):
        ...
        """
        self.ch_session.current_limit = current_limit
    ...
    def cs_configure_single_point_force_dc_voltage_asymmetric_limits(
        ...
        """
        self.ch_session.source_mode = nidcpower.SourceMode.SINGLE_POINT
        self.ch_session.output_function = nidcpower.OutputFunction.DC_VOLTAGE
        self.ch_session.voltage_level = voltage_level
        self.ch_session.current_limit_high = current_limit_high
        self.ch_session.current_limit_low = current_limit_low
        v_value = voltage_level_range
        if v_value == 0.0:
            v_value = abs(voltage_level)
        self.ch_session.voltage_level_range = v_value
        c_value = current_limit_range
        if c_value == 0.0:
            c_value = max(abs(current_limit_high), abs(current_limit_low))
        self.ch_session.current_limit_range = c_value
        self.ch_session.compliance_limit_symmetry = nidcpower.ComplianceLimitSymmetry.ASYMMETRIC
    ...
    def cs_configure_voltage_level_range(self, voltage_level_range=0.0):
        ...
        valid values to which the voltage level can be set. Use the voltage_level_auto-range
        ...
        """
        self.ch_session.voltage_level_range = voltage_level_range
    ...
    def cs_configure_source_adapt(
        ...
        """
        self.ch_session.transient_response = enums.TransientResponse.CUSTOM
        self.ch_session.voltage_gain_bandwidth = voltage_ctr.gain_bandwidth
        self.ch_session.voltage_compensation_frequency = voltage_ctr.compensation_frequency
        self.ch_session.voltage_pole_zero_ratio = voltage_ctr.pole_zero_ratio
        self.ch_session.current_gain_bandwidth = current_ctr.gain_bandwidth
        self.ch_session.current_compensation_frequency = current_ctr.compensation_frequency
        self.ch_session.current_pole_zero_ratio = current_ctr.pole_zero_ratio
    ...
    def cs_get_source_adapt_settings(self):
        ...
        """
        transient_response = self.ch_session.transient_response
        v_gain_bw = self.ch_session.voltage_gain_bandwidth
        v_comp_fr = self.ch_session.voltage_compensation_frequency
        v_pole_0_ratio = self.ch_session.voltage_pole_zero_ratio
        i_gain_bw = self.ch_session.current_gain_bandwidth
        i_comp_fr = self.ch_session.current_compensation_frequency
        i_pole_0_ratio = self.ch_session.current_pole_zero_ratio
    ...
    def cs_configure_output_connected(self, output_connected=False):
        ...
        """
        self.ch_session.output_connected = output_connected
    ...
    def cs_configure_output_enabled(self, output_enabled=False):
        ...
        Args:
            ...
            False.
        """
        self.ch_session.output_enabled = output_enabled
    ...
    def cs_configure_output_function(self, output_function=nidcpower.OutputFunction.DC_VOLTAGE):
        ...
        """
        self.ch_session.output_function = output_function
    ...
    def cs_configure_output_resistance(self, output_resistance=0.0):
        ...
        """
        self.ch_session.output_resistance = output_resistance
    ...
    def cs_configure_source_mode(self, source_mode=nidcpower.SourceMode.SINGLE_POINT):
        ...
        """
        self.ch_session.source_mode = source_mode
    ...
    def cs_set_sequence(self, values: list, source_delays: list):
        """
        Set sequence values and source delays

        Args:
            values (list): Sequence values
            source_delays (list): Source delays in seconds, must be positive.
        """

        self.ch_session.set_sequence(values, source_delays)
    ...
    def cs_configure_measurements(self, mode=MeasurementMode.AUTO):
        ...
        if mode == MeasurementMode.SOFTWARE_TRIGGER:
            self.ch_session.measure_when = nidcpower.MeasureWhen.ON_MEASURE_TRIGGER
            self.ch_session.measure_trigger_type = nidcpower.TriggerType.SOFTWARE_EDGE
            self.ch_session.measure_record_length = 1
        else:
            self.ch_session.measure_when = nidcpower.MeasureWhen.ON_DEMAND
    ...
    def cs_configure_export_signal(self, signal: enums.SendSoftwareEdgeTriggerType, terminal):
        ...
        if signal == enums.SendSoftwareEdgeTriggerType.START:
            self.ch_session.exported_start_trigger_output_terminal = terminal
            terminal_name = self.ch_session.exported_start_trigger_output_terminal
        elif signal == enums.SendSoftwareEdgeTriggerType.SOURCE:
            self.ch_session.exported_source_trigger_output_terminal = terminal
            terminal_name = self.ch_session.exported_source_trigger_output_terminal
        elif signal == enums.SendSoftwareEdgeTriggerType.MEASURE:
            self.ch_session.exported_measure_trigger_output_terminal = terminal
            terminal_name = self.ch_session.exported_measure_trigger_output_terminal
        elif signal == enums.SendSoftwareEdgeTriggerType.SEQUENCE_ADVANCE:
            self.ch_session.exported_sequence_advance_trigger_output_terminal = terminal
            terminal_name = self.ch_session.exported_sequence_advance_trigger_output_terminal
        elif signal == enums.SendSoftwareEdgeTriggerType.PULSE:
            self.ch_session.exported_pulse_trigger_output_terminal = terminal
            terminal_name = self.ch_session.exported_pulse_trigger_output_terminal
        else:
            terminal_name = terminal
        return terminal_name

    def cs_send_sw_edge_trigger(self, trigger_to_send=enums.SendSoftwareEdgeTriggerType.MEASURE):
        """
        ...
        Args:
            ...
            **Defined Values:**
            ...
            +----------------------------------------+---------------------------------------+
        """
        return self.ch_session.send_software_edge_trigger(trigger_to_send)
    ...
    def cs_wait_for_event(self, event=nidcpower.Event.SOURCE_COMPLETE, timeout=10.0):
        ...
        """
        return self.ch_session.wait_for_event(event, timeout)

    def cs_configure_and_commit_waveform_acquisition(
        self, sample_rate=1000000000, buffer_length_s=1.0
    ):
        """
        ...
        Args:
            ...
            Defaults to 1GigaHz = 1000000000
            buffer_length_s (float, optional): Number of samples to hold in memory.
            Defaults to 1.0 sec.
        ...
        self.ch_session.aperture_time_units = enums.ApertureTimeUnits.SECONDS
        self.ch_session.aperture_time = 1 / sample_rate
        self.ch_session.measure_record_length = 1  # need to see if we can set 2
        self.ch_session.measure_record_length_is_finite = True
        self.ch_session.measure_when = nidcpower.MeasureWhen.ON_MEASURE_TRIGGER
        self.ch_session.measure_trigger_type = nidcpower.TriggerType.SOFTWARE_EDGE
        self.ch_session.commit()
        try:
            total_sec = self.ch_session.measure_record_delta_time.total_seconds()
        except Exception:
            total_sec = 1 / sample_rate
        num_samples = int(math.ceil(buffer_length_s / total_sec))
        # coerce num_samples to be between 1 and max value of I32 (2147483647)
        if num_samples < 1:
            num_samples = 1
        elif num_samples > 2147483647:
            num_samples = 2147483647
        if self.ch_session.measure_buffer_size < num_samples:
            self.ch_session.measure_buffer_size = num_samples
        self.ch_session.measure_record_length = num_samples
        self.ch_session.commit()
        return settings
    ...
    def cs_get_measurement_settings(self):
        ...
        """
        settings = {
            "aperture_time_units": self.ch_session.aperture_time_units,
            "aperture_time": self.ch_session.aperture_time,
            "measure_when": self.ch_session.measure_when,
            "measure_trigger_type": self.ch_session.measure_trigger_type,
            "measure_record_length": self.ch_session.measure_record_length,
            "measure_record_length_is_finite": self.ch_session.measure_record_length_is_finite,
        }
        return settings
    ...
    def cs_set_measurement_settings(self, settings):
        ...
        """
        self.ch_session.aperture_time = settings["aperture_time"]
        self.ch_session.aperture_time_units = settings["aperture_time_units"]
        self.ch_session.measure_when = settings["measure_when"]
        self.ch_session.measure_trigger_type = settings["measure_trigger_type"]
        self.ch_session.measure_record_length = settings["measure_record_length"]
        self.ch_session.measure_record_length_is_finite = settings[
    ...
    def cs_measure_setup(self, measurement_mode: MeasurementMode):
        ...
        if measurement_mode == MeasurementMode.MEASURE_MULTIPLE:
            ...
            fetch_or_measure = self.ch_session.measure_when == enums.MeasureWhen.ON_MEASURE_TRIGGER

        if fetch_or_measure:
            self.ch_session.send_software_edge_trigger(enums.SendSoftwareEdgeTriggerType.MEASURE)
    ...
    def cs_measure_execute(self, fetch_or_measure: bool):
        ...
        """
        if fetch_or_measure:
            samples = self.ch_session.fetch_multiple(1, 1.0)
        else:
            samples = self.ch_session.measure_multiple()
        voltages = []
        currents = []
        in_compliance = []
        for s in samples:
        ...
        return voltages, currents, in_compliance
    ...
    def cs_get_properties(self):
        ...
        for pin, channel in zip(pns, channels):
            ...
            if match in ["4110", "4130"]:
                ...
                curr_lim_h = " - "
                curr_lim_l = " - "
            elif match in ["4112", "4113", "4132"]:
                tr_response = " - "
                output_connected = str(ss.output_connected)
                ap_time = str(ss.aperture_time)
                volt_lim_h = " - "
                volt_lim_l = " - "
                curr_lim_h = " - "
                curr_lim_l = " - "
            elif match in ["4135", "4136", "4137", "4138", "4139", "4147", "4190"]:
                tr_response = str(ss.transient_response)
                output_connected = str(ss.output_connected)
                ap_time = str(ss.aperture_time)
                volt_lim_h = str(ss.voltage_limit_high)
                volt_lim_l = str(ss.voltage_limit_low)
                curr_lim_h = str(ss.current_limit_high)
                curr_lim_l = str(ss.current_limit_low)
            elif match in ["4140", "4141", "4142", "4143", "4144", "4145"]:
                tr_response = str(ss.transient_response)
                output_connected = " - "
                ap_time = str(ss.aperture_time)
                volt_lim_h = str(ss.voltage_limit_high)
            ...
            ch_prop = ChannelProperties(
                self.session.instruments[max_instr_name].instrument_manufacturer,
                model,
                channel,
                pin,
                output_function,
                ...
                ss.output_enabled,
                output_connected,
                ss.voltage_level if "voltage_level" in list(ss.__dir__()) else " - ",
                ss.voltage_limit if "voltage_limit" in list(ss.__dir__()) else " - ",
                volt_lim_h,
                volt_lim_l,
                ss.voltage_limit_range if "voltage_limit_range" in list(ss.__dir__()) else " - ",
                ss.current_level if "current_leve" in list(ss.__dir__()) else " - ",
                ss.current_limit if "current_limit" in list(ss.__dir__()) else " - ",
                curr_lim_h,
                curr_lim_l,
                ss.current_limit_range if "current_limit_range" in list(ss.__dir__()) else " - ",
            )
...
class _NIDCPowerTSM:
    ...
    @staticmethod
    def _parse_instrument_names(resource_string: str):
        """
    ...
    def _configure_settings_array(
        ...
        Args:
            ...
            transient_responses (list of enum): Controls how to control the response based on load.
                Defaults to enums.TransientResponse.NORMAL.
        """
        for (ssc, ap_t, src_delay, sense, ap_t_unit, t_r,) in zip(
            self._sscs,
            aperture_times,
            source_delays,
            senses,
            ...
            ssc.cs_configure_settings(ap_t, src_delay, sense, ap_t_unit, t_r)
    ...
    def _force_voltage_asymmetric_limits(
        ...
        self._force_voltage_asymmetric_limits_array(
            voltage_levels,
            voltage_level_ranges,
            current_limit_highs,
            current_limit_lows,
    ...
    def configure_aperture_time_with_abort_and_initiate(
        ...
        """
        for ssc in self._sscs:
            ssc.cs_config_aperture_restart(aperture_time, aperture_time_units)
        return
    ...
    def configure_output_function(self, output_function=nidcpower.OutputFunction.DC_VOLTAGE):
        ...
        Configures the method to generate on the specified channel(s).
        When OutputFunction.DC_VOLTAGE is selected, the device generates the desired voltage level
        on the output as long as the output current is below the current limit.

        When OutputFunction.DC_CURRENT is selected, the device generates the desired current level
        on the output as long as the output voltage is below the voltage limit.
        ...
        """
        for ssc in self._sscs:
            ssc.cs_configure_output_function(output_function)

    def set_sequence(self, values, source_delays):
        """
        Configures a series of voltage or current outputs and corresponding source delays for the
        sessions. The source mode must be set to Sequence for this function to take effect.
        ...
        """
        for ssc in self._sscs:
            ssc.cs_set_sequence(values, source_delays)
    ...
    def clear_triggers(self):
        """
        clears the trigger settings
        """
        self.abort()
        self.configure_measurements()
        for ssc in self._sscs:
            ssc.ch_session.digital_edge_measure_trigger_input_terminal = ""
            # ssc.ch_session.start_trigger_type = nidcpower.TriggerType.NONE
            # ssc.ch_session.sequence_advance_trigger_type = nidcpower.TriggerType.NONE
            # ssc.ch_session.source_trigger_type = nidcpower.TriggerType.NONE
            # ssc.ch_session.measure_trigger_type = nidcpower.TriggerType.SOFTWARE_EDGE
            # ssc.ch_session.measure_when = nidcpower.MeasureWhen.ON_DEMAND
            # ssc.ch_session.measure_record_length_is_finite = True
            # ssc.ch_session.measure_record_length = 1
            # Need to review these settings to be enabled or not for clearing triggers
        self.commit()

    def configure_and_start_waveform_acquisition_ext_trigger(
        self, trigger_line: str, sample_rate=1000000000.0, buffer_length=1.0
    ):
        """Configure measure on external trigger

        Args:
            trigger_line (str): valid pxi trigger line on which trigger is received.
            sample_rate (float, optional): sampling rate for acquisition.
            Defaults to 1GigaHz = 1000000000.0.
            buffer_length (float, optional): length of measurement time to be stored in memory. Defaults to 1.0 sec.
        ...
        previous_settings = []
        for ssc in self._sscs:
            ssc.ch_session.abort()
            settings = ssc.cs_configure_and_commit_waveform_acquisition(sample_rate, buffer_length)
            previous_settings.append(settings)
            ssc.ch_session.measure_when = enums.MeasureWhen.ON_MEASURE_TRIGGER
            ssc.ch_session.measure_trigger_type = enums.TriggerType.DIGITAL_EDGE
            ssc.ch_session.digital_edge_measure_trigger_input_terminal = trigger_line
            ssc.ch_session.commit()
            # Trigger edge is configured to Rising by default
        self.initiate()
        start_time = datetime.now()
        settings = [previous_settings, start_time]
        return settings

    def configure_and_start_waveform_acquisition(self, sample_rate=1000000000.0, buffer_length=1.0):
        """
        ...
        Args:
            sample_rate (float, optional): sampling rate for acquisition.
            Defaults to 1GigaHz = 1000000000.0.
            buffer_length (float, optional): length of measurement time to be stored in memory.
            Defaults to 1.0 sec.
    ...
    def send_software_edge_trigger(
        ...
        """
        for ssc in self._sscs:
            ssc.cs_send_sw_edge_trigger(trigger_to_send)
    ...
    def fetch_waveform(self, waveform_t0, waveform_length_s=0.0):
        ...
        current_waveforms = []
        for ssc in self._sscs:
            record_dt = ssc.cs_session.measure_record_delta_time.total_seconds()
            fetch_backlog = ssc.cs_session.fetch_backlog
            if waveform_length_s == 0.0:
                fetch_samples = fetch_backlog
            else:
                fetch_samples = int(waveform_length_s / record_dt)
            # samples = ssc.cs_session.fetch_multiple(fetch_samples, timeout=waveform_length_s + 1)
            # voltages = []
            # currents = []
            # in_compliance = []
            # for s in samples:
            ...
            channels = ssc.cs_channels.split(",")
            for ch in channels:
                samples = ssc.session.channels[ch].fetch_multiple(
    ...
    def measure(self, measurement_mode=MeasurementMode.AUTO, compliance_check=False):
        """
        ...
        Args:
            ...
            MeasurementMode.AUTO.
            compliance_check (Boolean, optional): specifies the compliance check to be performed or
            not. it affects the output tuple.

        Returns:
            voltages, currents, [compliance]: tuple of list of voltages and currents, optionally compliance list
        """
        ...
        i = 0
        for ssc in self._sscs:
            voltages_new, currents_new, compliance_new = ssc.cs_measure_execute(
                fetch_or_measure_array[i]
            )
            voltages += voltages_new
            currents += currents_new
            compliance += compliance_new
            i += 1
        if compliance_check:
    ...
    def filter_sites(self, requested_sites):
        ...
        for ssc in self._sscs:
            found = False
            sites = common.pin_list_to_sites_pins(ssc.cs_channels)[2]
            for s in sites:
    ...
    def filter_pins(self, requested_pins):
        ...
        temp2 = []
        for ssc in self._sscs:
            sites_pins, pins, sites = common.pin_list_to_sites_pins(ssc.cs_channels)
            try:
...
def filter_pins(dc_power_tsm: TSMDCPower, desired_pins):
    ...
    for d_pin in desired_pins:
        ...
        i += 1
    dut_pins, system_pins = common.get_dut_and_system_pins_from_expanded_pin_list(pins_expand_new)
...
def pins_to_sessions(
    ...
    Returns:
        TSMDCPower: object containing pin-query context variable and other session details
    """
    if len(sites) == 0:
...
@nitsm.codemoduleapi.code_module
def initialize_sessions(tsm: SMContext, lock: Lock = None, power_line_frequency=60.0, **kwargs):
    """
